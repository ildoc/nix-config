...

{
  ...
  # ============================================================================
  # SERVIZIO PER SETUP WIREGUARD CONFIG
  # ============================================================================
  systemd.services.setup-wireguard-config = lib.mkIf (config.networking.hostName == "slimbook") {
    ...
    script = ''
      ...
      # Usa valori hardcoded che vengono dalla configurazione centralizzata
      # Questi valori devono corrispondere a quelli in modules/config/vpn.nix
      VPN_NAME="${config.myConfig.vpn.connectionName or "office-vpn"}"
      CONFIG_FILE="${config.myConfig.vpn.configFile or "wg0.conf"}"
      VPN_INTERFACE="${config.myConfig.vpn.interface or "wg0"}"
      VPN_DESCRIPTION="${config.myConfig.vpn.description or "Office VPN Connection"}"
      
      echo "VPN Name: $VPN_NAME"
      echo "Config File: $CONFIG_FILE"
      echo "Interface: $VPN_INTERFACE" 
      echo "Description: $VPN_DESCRIPTION"
      ...
      cp /run/secrets/wireguard_config "/etc/wireguard/$CONFIG_FILE"
      chmod 600 "/etc/wireguard/$CONFIG_FILE"
      chown root:root "/etc/wireguard/$CONFIG_FILE"
      
      # Verifica che il file sia leggibile
      if [ -r "/etc/wireguard/$CONFIG_FILE" ]; then
        echo "✓ WireGuard configuration prepared for NetworkManager"
        echo "✓ Connection name: $VPN_NAME"
        echo "✓ Config file: $(ls -la "/etc/wireguard/$CONFIG_FILE")"
        echo "✓ Config preview: $(head -1 "/etc/wireguard/$CONFIG_FILE")"
        echo "✓ Ready for NetworkManager import as '$VPN_NAME'"
      else
